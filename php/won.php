<?php
  ...
  if (isset ($_SESSION['ok'])) {
    ...
    if ($stage == 1 || $stage == 2) {
      ...
      $result = mysqli_query($conn, $query);
      $row = mysqli_fetch_array($result); $max = $row['0'];
      $query = "SELECT wins FROM ".$activitate." WHERE id_user = '$id' AND level = '$level'";
      $result = mysqli_query($conn, $query);
      $row = mysqli_fetch_array($result); $wins = $row['0'];
      $wins++;
      ...
      $result = mysqli_query($conn, $query);
      $row = mysqli_fetch_array($result); $oldpoints = $row['0'];
      if ($points > $oldpoints) {
      ...
      $result = mysqli_query($conn, $query);
      $row = mysqli_fetch_array($result); $oldtime = $row['0'];
